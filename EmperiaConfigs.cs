...
namespace Emperia
{
    public class EmperiaConfig : ModConfig
	{
		...
		//[Header("Header")] // Headers are like titles in a config. You only need to declare a header on the item it should appear over, not every item in the category.
		[Label("Indicate Items with Added Recipes")] // A label is the text displayed next to the option. This should usually be a short description of what it does.
		//Indicator for Emperia Materials
		//Indicate Items with New Recipes
		[Tooltip("Displays a crown symbol in the tooltip of vanilla items if they are used for crafting recipes added by Emperia")] // A tooltip is a description showed when you hover your mouse over the option. It can be used as a more in-depth explanation of the option.
		[DefaultValue(true)] // This sets the configs default value.
		//[ReloadRequired] // Marking it with [ReloadRequired] makes tModLoader force a mod reload if the option is changed. It should be used for things like item toggles, which only take effect during mod loading
		public bool EmperiaCraftingIndicator;
	}
}