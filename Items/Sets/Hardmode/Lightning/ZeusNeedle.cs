...
namespace Emperia.Items.Sets.Hardmode.Lightning
{
    public class ZeusNeedle : ModItem
    {
		...
		public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockBack)
        {
            ...
            int num250 = Dust.NewDust(new Vector2(player.position.X, player.position.Y - 16), 8, 8, DustID.Electric, (float)(player.direction * 2), 0f, 226, new Color(53f, 67f, 253f), 1.3f);
            return true;
        }
    }
}
