...
namespace Emperia.Projectiles
{
	public class PaintBall : ModProjectile
	{
		...
		public override void AI()
		{
			...
			if (timer == 4)
			{
				for (int i = 0; i < 2; ++i)
			    {
			        if (i < 1)
			        {
					    int index2 = Dust.NewDust(new Vector2((float)(Projectile.position.X + 4.0), (float)(Projectile.position.Y + 4.0)), Projectile.width - 8, Projectile.height - 8, DustID.Snow, direction.X, direction.Y, 0, rgb, 1.3f);
              			Main.dust[index2].position = Projectile.Center;
               			Main.dust[index2].noGravity = true;
			        }
					else 
					{
					    int index2 = Dust.NewDust(new Vector2((float)(Projectile.position.X + 4.0), (float)(Projectile.position.Y + 4.0)), Projectile.width - 8, Projectile.height - 8, DustID.Snow, direction2.X, direction2.Y, 0, rgb, 1.3f);
              			Main.dust[index2].position = Projectile.Center;
               			Main.dust[index2].noGravity = true;
					}
			    }
		...
		public override void OnKill(int timeLeft)
        {
			...
			int index3 = Dust.NewDust(new Vector2((float) (Projectile.position.X + 4.0), (float) (Projectile.position.Y + 4.0)), Projectile.width - 8, Projectile.height - 8, DustID.Snow, 0.0f, 0.0f, 0, rgb, 1f);
