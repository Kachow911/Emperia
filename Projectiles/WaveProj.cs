...
namespace Emperia.Projectiles
{
    ...
    public class WaveProj : ModProjectile
    {
        ...
        {
            timer += 8;
			int dust = Dust.NewDust(Projectile.position + new Vector2(10f * (float)Math.Cos(MathHelper.ToRadians(timer)), 0f).RotatedBy(Projectile.rotation + 1.57), 8, 8, DustID.Snow, 0f, 0f, 91, new Color(83, 66, 180), 1f);
            int dust1 = Dust.NewDust(Projectile.position + new Vector2(10f * (float)Math.Cos(MathHelper.ToRadians(timer)), 0f).RotatedBy(Projectile.rotation - 1.57), 8, 8, DustID.Snow, 0f, 0f, 91, new Color(83, 66, 180), 1f);
		...
		public override void OnKill(int timeLeft)
        {
			for (int i = 0; i < 360; i += 10)
			{
				...
				vec.Normalize();
				int num622 = Dust.NewDust(Projectile.position, 8, 8, DustID.Snow, 0f, 0f, 91, new Color(83, 66, 180), 1.5f);
