...
namespace Emperia.Npcs.Inquisitor
{
    public class Inquisitor : ModNPC
    {
        ...
        public override void AI()
        {
			...
			if (masksToSpawn > 0)
			{
				NPC.NewNPC((int)NPC.Center.X, (int)NPC.Center.Y, NPCType<AgonyMask>(), ai0: NPC.whoAmI, ai1: masksToSpawn);
				masksToSpawn--;
			}
            if (NPC.ai[3] == 0 && IsInPhaseOne())
            {
				SetMove(Move.Hover, 240);
                NPC.NewNPC((int)NPC.Center.X - 20, (int)NPC.Center.Y, NPCType<EocPuppet>(), ai0: player.whoAmI, ai1: NPC.whoAmI);
			...
			} else if (move == Move.Shoot)
			{
				...
				if (counter % 10 == 0)
				{
					...
					direction.Normalize();
					Projectile.NewProjectile(NPC.GetProjectileSpawnSource(), placePosition.X, placePosition.Y, direction.X * 7f, direction.Y * 7f, ModContent.ProjectileType<FearBolt>(), 30, 1, Main.myPlayer, 0, 0);	
				}
				if (counter <= 0)
				{
					SetMove(Move.Hover, 120);
