...
namespace Emperia.Npcs.GoblinArmy
{
    public class GoblinGiant : ModNPC
    {
        ...
        public override void AI()
		{
			...
			if (goblinCounter <= 0)
            {
				NPC.NewNPC(NPC.GetSource_FromAI(), (int)NPC.Center.X + Main.rand.Next(-25, 25), (int)NPC.Center.Y + 40, NPCType<GoblinRamCarrier>());
				NPC.NewNPC(NPC.GetSource_FromAI(), (int)NPC.Center.X + Main.rand.Next(-25, 25), (int)NPC.Center.Y + 40, NPCType<GoblinRamCarrier>());
				NPC.NewNPC(NPC.GetSource_FromAI(), (int)NPC.Center.X + Main.rand.Next(-25, 25), (int)NPC.Center.Y + 40, NPCType<GoblinRamCarrier>());
				goblinCounter = 600;
            }
       ...
       /* private void SmoothMoveToPosition(Vector2 toPosition, float addSpeed, float maxSpeed, float slowRange = 64, float slowBy = .95f)
        ...
        {
			Gore.NewGore(NPC.GetSource_Death(), NPC.position, NPC.velocity, ModContent.Find<ModGore>("Gores/Goblin/GoblinGiantGoreHead").Type, 1f);
			Gore.NewGore(NPC.GetSource_Death(), NPC.position, NPC.velocity, ModContent.Find<ModGore>("Gores/Goblin/GoblinGiantGoreLeg").Type, 1f);
			Gore.NewGore(NPC.GetSource_Death(), NPC.position, NPC.velocity, ModContent.Find<ModGore>("Gores/Goblin/GoblinGiantGoreArm").Type, 1f);
			Gore.NewGore(NPC.GetSource_Death(), NPC.position, NPC.velocity, ModContent.Find<ModGore>("Gores/Goblin/GoblinGiantGoreCannon_4").Type, 1f);
			Gore.NewGore(NPC.GetSource_Death(), NPC.position, NPC.velocity, ModContent.Find<ModGore>("Gores/Goblin/GoblinGiantGoreCannon_3").Type, 1f);
			Gore.NewGore(NPC.GetSource_Death(), NPC.position, NPC.velocity, ModContent.Find<ModGore>("Gores/Goblin/GoblinGiantGoreCannon_1").Type, 1f);
		...
		{
			Item.NewItem(NPC.GetSource_Loot(), (int)NPC.position.X, (int)NPC.position.Y, NPC.width, NPC.height, ModContent.ItemType<Items.GiantPlating>(), Main.rand.Next(3, 8));
				
				if (Main.rand.Next(5) == 0)
				{
					Item.NewItem(NPC.GetSource_Loot(), (int)NPC.position.X, (int)NPC.position.Y, NPC.width, NPC.height, ModContent.ItemType<GiantsDagger>());
				}
				if (Main.rand.Next(5) == 0)
				{
				Item.NewItem(NPC.GetSource_Loot(), (int)NPC.position.X, (int)NPC.position.Y, NPC.width, NPC.height, ModContent.ItemType<GiantsDevastator>());
				}
				if (Main.rand.Next(5) == 0)
				{
				Item.NewItem(NPC.GetSource_Loot(), (int)NPC.position.X, (int)NPC.position.Y, NPC.width, NPC.height, ModContent.ItemType<OversizedFemur>());
				}
			if (Main.rand.Next(5) == 0)
			{
				Item.NewItem(NPC.GetSource_Loot(), (int)NPC.position.X, (int)NPC.position.Y, NPC.width, NPC.height, ModContent.ItemType<GiantsHead>());
			}
		}
